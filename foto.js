...
const User = [
	...
	},
	{
		type:'input',
		name:'text',
		message:'Isi Text Comment 1 (Gunakan Pemisah [|] bila lebih dari 1)',
		validate: function(value){
			if(!value) return 'Can\'t Empty';
			return true;
		}
...
const CommentAndLike = async function(session, accountId, text){
	...
	return chalk`{bold.cyan Post Kosong (SKIPPED)}`
};
...
const Excute = async function(User, TargetUsername, Text, Sleep){
	try {
		...
		console.log(chalk`{green  | Widih sukses, try to get Followers Target ....}`)
		...
		for (var i = 0; i < TargetResult.length; i++) {
			...
			console.log(chalk`{yellow Tunggu dulu cok ${Sleep} MiliSeconds}`);
			await delay(Sleep);
		}
	} catch (err) {
		console.log(err);
...
console.log(chalk`
{bold Instagram FLMT Auto Comment, Auto Like}
{green BC0DE.NET - NAONLAH.NET - WingKocoli}
{bold.red Code By Gu1t4r_Id | ahmadrizki229@gmail.com}
`);
